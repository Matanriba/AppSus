...
let gSearch = storageService.loadFromStorage('searchDB') || [];

let gBooks = storageService.loadFromStorage('booksDB') || [{
        id: "OXeMG8wNskc",
...
function query(filterBy) {
    ...
    if (filterBy) {
        ...
        const booksToDisplay = gBooks.filter(book => book.title.includes(title) &&
...
function getAdjBookId(bookId, diff) {
    const bookIdx = gBooks.findIndex(book => book.id === bookId)
    if (bookIdx === 0) return
    let nextBookIdx = bookIdx + diff
    if (nextBookIdx === gBooks.length) nextBookIdx = 0
    if (nextBookIdx === 0) nextBookIdx = gBooks.length - 1
    return gBooks[nextBookIdx].id
}

function _createReview({ name, rate, readAt, txt }) {
    return {
...
function addReview(bookId, review) {
    ...
    const currReview = _createReview(review)
...
function getBooksFromGoogle(search) {
    const searchInCache = gSearch.find((item) => Object.keys(item)[0] === search);
    if (searchInCache) {
        console.log('from cache');
        return Promise.resolve(Object.values(searchInCache)[0]);
    }
    console.log('from server');
    const url = `https://www.googleapis.com/books/v1/volumes?printType=books&q=${search}`;
    return axios.get(url).then((res) => {
        const { items } = res.data;
        const mappedRes = mapAPIRes(items);
        gSearch.push({
            [search]: mappedRes
        });
        storageService.saveToStorage('searchDB', gSearch);
        return mappedRes;
    });
}

function mapAPIRes(items) {
    return items.map((item) => ({
        ...
        reviews: [],
        listPrice: {
            amount: utilService.getRandomIntInclusive(50, 150),
            currencyCode: (Math.random() <= 0.5) ? 'USD' : 'ILS',
